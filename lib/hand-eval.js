...
// You may pass a resultType as the second argument to check only for that result.
function getHandResult(hand, resultType) {
	let evalContext = getEvalContext(hand);
	let evaluatorsToCheck;
	if (resultType) {
		if (!evaluators[resultType] || !evaluators[resultType].isResult) {
			throw new XError(XError.INVALID_ARGUMENT, 'Invalid resultType: ' + resultType);
		}
		evaluatorsToCheck = [ resultType ];
	} else {
		evaluatorsToCheck = resultEvaluatorOrder;
	}

	for (let evaluatorId of evaluatorsToCheck) {
		let evaluator = evaluators[evaluatorId];
		let result = evaluator.evaluate(evalContext);
		if (result) {
			result.type = evaluatorId;
			if (resultType) result.forcedResultType = true;
			return result;
		}
	}
	return null;
